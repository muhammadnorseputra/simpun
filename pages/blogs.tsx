...
const Blogs: NextPage = ({ featured: {items}, postlist }: any) => {
  const { images, title, author, labels, published: date } = items[0]
    ...
    <article key={post.id} className="mx-8 my-6 relative overflow-hidden group">
      <div className="flex justify-between items-center">
        <div className="flex flex-col justify-start items-start space-y-2 w-9/12 pr-5">
          ...
          <h2 className="font-bold text-md text-slate-600 dark:text-slate-800 dark:hover:text-green-500 hover:text-green-500 line-clamp-2">{post.title}</h2>
        ...
        <div className="relative">
        <Image layout="intrinsic" width="110" 
          height="110"
        ...
        <div className="transition-all absolute top-[calc(35%)] left-[40%] hidden hover:scale-110 group-hover:block  p-2 rounded-full bg-white"><LinkIcon className="w-4 h-4 text-green-600 dark:text-amber-500"/></div>
        </div>
      </div>
    </article>
    ))}
  ...
  }} className="bg-slate-200 hover:bg-gray-100 text-gray-800 font-semibold py-2 px-4 rounded inline-flex items-center" type="button">
    <ArrowsExpandIcon className="w-4 h-4 text-gray-500 dark:text-amber-500 mr-2"/> Loadmore
    </button>
    </div>
  </Layouts>
  );
