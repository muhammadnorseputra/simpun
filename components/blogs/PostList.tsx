import { fetchImageId, tglIndo, toBase64 } from "@/utils/index";
...
export default function PostList({posts}: any) {
    return (
        posts.slice(1).map((post:any) => (
            <article key={post.id} className="mx-6 md:mx-6 my-6 relative overflow-hidden group">
              <div className="flex justify-between items-center">
                <div className="flex flex-col justify-start items-start space-y-2 w-9/12 pr-5">
                  
                  <div className="flex justify-start items-center space-x-2">
                  <div className="px-3 py-1 bg-slate-800/50 text-xs text-white rounded-xl">{post.labels[0]}</div> 
                  </div>
                  ...
                  <div className="text-sm text-slate-500 flex justify-start items-center space-x-1"> 
                    <span className="text-slate-500 dark:text-slate-500 text-xs md:text-sm border-r pr-3 mr-2">{tglIndo(post.published)}</span>
