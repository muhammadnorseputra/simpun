...
export default function Project({ projects }: any) {
  ...
  return (
    <section id="projects" className='overflow-x-hidden border-b-8 dark:border-gray-800 border-gray-100'>
      <div className="relative px-6 py-12 min-h-[300px] bg-white dark:bg-green-500 shadow-t-2xl -mt-5 rounded-t-2xl rounded-b-2xl dark:rounded-b-none overflow-hidden z-10" data-aos="fade-in" data-aos-delay="150">
        ...
        <button onClick={() => slider?.current?.slickNext()} data-aos="fade-down" data-aos-delay="250" className="bg-white transition-all group text-black hover:text-green-700 rounded-full absolute right-8 top-10 p-2" type="button" role="button"> <ArrowCircleRightIcon className="w-6 h-6 transition-all group-active:scale-75"/> </button>
        <button onClick={() => slider?.current?.slickPrev()} data-aos="fade-down" data-aos-delay="150" className="bg-white transition-all group text-black hover:text-green-700 rounded-full absolute right-20 top-10 p-2" type="button" role="button"> <ArrowCircleLeftIcon className="w-6 h-6 transition-all group-active:scale-75"/> </button>
      </div>
    </section>
  );
}
